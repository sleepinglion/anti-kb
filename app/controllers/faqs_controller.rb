class FaqsController < BoardController
  ...
  def create
    ...
    respond_to do |format|
      if @faq.save
        format.html { redirect_to faqs_url(:faq_category_id=>@faq.faq_category.id), notice: @controller_name +t(:message_success_insert)}
  ...
  private
    ...
    def faq_params
      params.require(:faq).permit(:id,:faq_category_id,:title,faq_content_attributes: [:id,:content])
    end
end
